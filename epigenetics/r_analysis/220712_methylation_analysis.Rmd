...
nanopolish.meth <- "/pym/Data/paul/hg002_y/meth_calls/calls/nanonome_2.7/processed/hg002_cpg_filtered_meth.freq"
...
c.all <- cor(meth.human.combined,use = "complete.obs")
...
c.1 <- cor(meth.human.combined$sr_emseq_lab1, meth.human.combined$sr_emseq_lab2)
title.1 <- paste0("Nanopolish vs. WGBS"," ","(r = ",round(c.1,digits = 3),")")
plot.1 <- ggplot(meth.human.combined, aes(x=sr_emseq_lab1,y=sr_emseq_lab2)) +
  geom_bin_2d(bins=25) + scale_fill_gradientn(colors=r, trans="log10") +
  ylab("WGBS Meth. Frequency") +
  xlab("Nanopolish Meth. Frequency") +
  ggtitle(title.1) + 
