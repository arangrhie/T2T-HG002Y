...
files_to_read <- list.files(path = "/atium/Data/projects/emseq_smk/bismark/emseq/mbias",pattern = "M-bias",full.names = T)
...
ggplot(emseq.r2,aes(x=position,y=meth_perc,color=experiment)) + 
  ...
  theme_classic() + geom_vline(xintercept = 6)
...
# Methylseq
## Make list of files to read in
files_to_read <- list.files(path = "/atium/Data/projects/emseq_smk/bismark/methylseq/mbias",pattern = "M-bias",full.names = T)
...
ggplot(methylseq.r2,aes(x=position,y=meth_perc,color=experiment)) + 
  geom_line(size=1) + ylim(c(0,100)) + 
  theme_classic() + geom_vline(xintercept = 6) + ggtitle("Methylseq R2")
...
# Nanopore
## Make list of files to read in
files_to_read <- list.files(path = "/atium/Data/projects/emseq_smk/bismark/nanopore/mbias",pattern = "M-bias",full.names = T)

## Read in the files, name them after the SRA ID, and put experiment name in a column
for (i in 1:length(files_to_read)) {
  #print(file)
  name <- unlist(strsplit(files_to_read[i], "[/]|[.]"))[9]
  #print(x)
  x <- read_tsv(file = files_to_read[i],
            skip = 2,
            col_types = "nnnnn")
...
## Pull out SRA names - these will correspond to data frames above
sra_names <- strsplit(files_to_read, "[/]|[.]") %>% purrr::map(`[`,9) %>% unlist()
...
for (i in 1:length(sra_names)) {
  x <- get(sra_names[i]) %>% 
    dplyr::slice(1:250) %>% 
    dplyr::select(position,meth_perc=`% methylation`,experiment)
  nanopore.r1 <- rbind(nanopore.r1,x)
  rm(x)
}

ggplot(nanopore.r1,aes(x=position,y=meth_perc,color=experiment)) + 
  geom_line() + ylim(c(0,100)) + 
  theme_classic() + geom_vline(xintercept = 11)
